  ...
  router.get('/', async (req, res) => {
    try {
      const result = await pool.query(
        'SELECT id, name, email, speciality, area, bio, experience, rating, image, availability_schedule, is_available, latitude, longitude, service_radius, is_verified FROM workers WHERE is_active = true ORDER BY name ASC'
      );
